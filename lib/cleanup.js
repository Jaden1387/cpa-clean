...
export async function executeCleanup(payload, addLog = () => {}) {
  ...
  log("开始清理");
  log(`配置: 主动探测=${config.active_probe ? "开启" : "关闭"}, 探测并发上限=${config.probe_workers}, 删除并发上限=${config.delete_workers}, 最大探测数=${config.max_active_probes}`);

  const files = await listAuthFiles(config);
  log(`拉取账户列表成功，总数: ${files.length}`);
  ...
  if (config.active_probe && config.max_active_probes !== 0 && probeCandidates.length > 0) {
    const effectiveProbeWorkers = Math.max(1, Math.min(config.probe_workers, probeCandidates.length));
    log(`开始主动探测，候选 ${probeCandidates.length} 个，实际探测并发 ${effectiveProbeWorkers}`);
    let done = 0;
    const total = probeCandidates.length;

    await runPool(probeCandidates, effectiveProbeWorkers, async (fileObj) => {
      const name = String(fileObj?.name || "");
      const authIndex = String(fileObj?.auth_index || "").trim();
      if (!authIndex) {
        done += 1;
  ...
  log(`命中清理规则: ${matched.length}`);

  let deletedMain = 0;
  const failures = [];

  if (matched.length > 0) {
    const effectiveDeleteWorkers = Math.max(1, Math.min(config.delete_workers, matched.length));
    log(`主流程删除目标 ${matched.length} 个，实际删除并发 ${effectiveDeleteWorkers}`);
    let done = 0;
    const total = matched.length;

    await runPool(matched, effectiveDeleteWorkers, async (hit) => {
  ...
  try {
    ...
    if (targets401.length === 0) {
      ...
      const effectiveDelete401Workers = Math.max(1, Math.min(config.delete_workers, targets401.length));
      log(`401补删: 待删除 ${targets401.length} 个，实际删除并发 ${effectiveDelete401Workers}`);
      await runPool(targets401, effectiveDelete401Workers, async (hit) => {
        const { ok, error } = await deleteAuthFile(config, hit.name);
        if (ok) {
          deleted401 += 1;
  ...
  log(`完成: 扫描=${report.scanned_total}, 命中=${report.matched_total}, 主清理=${report.deleted_main}, 401补清理=${report.deleted_401}, 总清理=${report.deleted_total}`);
